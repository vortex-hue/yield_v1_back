...
#     data = result.json()
#     print(data)
#     with open('scan_fin_data.json', 'w', encoding='utf-8') as f:
...
async def call_api(url: str, queries=None):
    ...
    # Ensure base_api_url and query_params are defined outside this function or passed as parameters
    async with aiohttp.ClientSession() as client:
        try:
            async with client.get(f"{base_api_url}/{url}", params={**query_params, **queries}) as response:
...
#         print(f"No categories found for company {company['symbol']}.")
#         return None  # Or handle it in some other way, like assigning a default category
...
async def create_company(company: dict, db: Session):
    ...
    if not categories:
        # Create a new category based on sector if no categories are found
        print(f"No categories found for company {company['symbol']}. Creating a new category based on sector.")
        new_category = Category(category_id=str(uuid4()), name=company_profile['sector'],
                                market_cap=company_market_cap)
        db.add(new_category)
        db.commit()
        company_category = new_category
    else:
        company_category: Category = categories[len(categories) - 1]
        for category in categories:
            if company_market_cap >= category.market_cap:
...
async def create_or_update_stock_price(ratio: dict, stock_price_id: str, company_id: str, db: Session):
    """ Creates a new stock price record or updates an existing one """
    # Convert the date field to a datetime.date object
    date_str = ratio['date']
    date_obj = datetime.datetime.strptime(date_str, "%Y-%m-%d").date()  # Assuming the date is in 'YYYY-MM-DD' format
    ...
    insert_stock = StockPrice(stock_price_id=stock_price_id, company=company_id,
                              ...
                              pb_ratio=ratio['priceToBookRatio'], ps_ratio=ratio['priceToSalesRatio'],
                              gross_profit_margin=ratio['grossProfitMargin'],
                              dividend_yield=ratio['dividendYield'], date=date_obj)  # Use the converted date object
    db.add(insert_stock)
    db.commit()
    return insert_stock
...
async def create_or_update_financial(financial: dict, financial_id: str, company_id: str,
    ...
    """ Creates a new financials record or updates an existing one """
    # Convert the date field to a datetime.date object
    date_str = financial['date']
    date_obj = datetime.datetime.strptime(date_str, "%Y-%m-%d").date()  # Assuming the date is in 'YYYY-MM-DD' format

    insert_financial = Financial(financial_id=financial_id, company=company_id,
                                 growth_rate=financial['revenueGrowth'],
                                 income_statement_type='Annual', date=date_obj,  # Use the converted date object
                                 gross_profit=income_statement['grossProfit'],
                                 operating_expenses=income_statement['operatingExpenses'],
                                 ebitda=income_statement['ebitda'],
                                 net_income=income_statement['netIncome'],
